...
def run(cipher, rounds, magma_dir=MAGMA_DIR, output_dir=OUTPUT_DIR):
    """
    ...
    for index in range(1, cipher.n):
        ...
        for line_no, line in enumerate(lines, 1):
            ...
            if dw_vec[0] != F(0):
                # This trail violates Delta_w1 = 0; skip silently
                continue
            ...
            #     continue
            if rc != 0 or "License error" in stdout or "Out of memory" in stdout or "Killed" in stdout:
                print(f"\n[!!!] MAGMA SYSTEM ERROR DETECTED!")
                print(f"Check the output manually. Script retained at {script_path}")
...
if __name__ == "__main__":
    ...
    cipher = RC_BLS
    rounds = 2          # rounds before Bars (r0 in RC_round_last_bricks)

    success = run(cipher, rounds)
    if not success:
        print("Search exhausted without finding a collision.")
